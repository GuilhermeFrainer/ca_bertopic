...
from src.processing import BATCH_SIZE, process_dataset
...
def test_index_uniqueness_across_batches(tmp_path):
    ...
    long_text = ". ".join(
        ["This is a long sentence that should be split eventually"] * 20
    )

    df = pl.DataFrame(
        {
            "text": [
                long_text if i % 100 == 0 else f"Short text {i}"
