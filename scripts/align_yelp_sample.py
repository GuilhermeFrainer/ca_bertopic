...
def main():
    ...
    raw_lf = raw_lf.rename({col: col.lower().replace(" ", "_") for col in raw_lf.collect_schema().names()})
    ...
    if "date" in raw_lf.collect_schema().names():
        raw_lf = raw_lf.sort("date").unique(subset=["clean_text"], keep="first", maintain_order=True)
    else:
    ...
    logger.info(f"Filtering {len(clean_raw_df)} cleaned documents by {len(sampled_ids)} sampled IDs...")
