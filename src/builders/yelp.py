...
def write_parquet_files(
    ...
    for file in files:
        ...
        if not json_path.exists():
            logger.warning(f"JSON file not found: {json_path}. Skipping conversion for this file.")
            continue
...
def build(raw_dir: Path, interim_dir: Path, skip_convert: bool = False):
    ...
    if not all(p.exists() for p in [business_path, review_path, user_path]):
        raise FileNotFoundError("One or more required Parquet files are missing in raw_dir.")
